...
# Cargar la clave de Notion desde las variables de entorno o desde un archivo de configuración seguro.
...
def get_notion_data():
    ...
    """
    data = []
    databases = list_databases()  # Obtener la lista de bases de datos

    for title, database_id in databases:
        try:
            # Realiza la consulta a cada base de datos
            response = notion.databases.query(database_id=database_id)

            # Extraer los resultados
            for item in response.get('results', []):
                # Captura solo los campos ID y Link
                row = {
                    'ID': None,  # Inicializa como None
                    'Link': None  # Inicializa como None
                }

                # Verificar y obtener el ID
                try:
                    if item['properties']['ID']['rich_text']:
                        row['ID'] = item['properties']['ID']['rich_text'][0]['text']['content']
                except (IndexError, KeyError):
                    print(f"ID no disponible para el elemento: {item}")

                # Verificar y obtener el Link
                try:
                    if 'Link' in item['properties'] and item['properties']['Link']['url']:
